...
**进程显示名**：挂件统一以 `deepseek-balance.exe` 运行——首次启动会把系统 `powershell.exe` 复制一份改名放进状态目录 `%USERPROFILE%\.codex\deepseek-balance\`，之后（`-Action start`、双击启动器、开机自启）都自动切到它再跑，所以任务管理器里看到的是 **deepseek-balance**，不再混在 powershell 里。`-Action status` 会打印当前宿主；副本被安全软件拦掉时自动回退 `powershell.exe`，功能不受影响。窗口标题与托盘悬停提示仍是「DeepSeek 余额挂件」。
...
**两个大小滑块**：`挂件大小` 调的是鲸鱼与窗口基准（0.6–2.5 倍）；`气泡大小` 是以挂件大小为基准的相对倍数（0.5–1.5），只影响气泡，鲸鱼不变；气泡比窗口大时窗口会自动跟着放大，不会被裁掉。调尺寸时**窗口右下角固定不动**——气泡向上方、左侧扩展，鲸鱼和贴边位置不跑；因为气泡可能让窗口变成长方形，吸附贴边时两个轴各按自己的宽/高对齐，不会留缝。
...
**角标怎么用**：角标 = 彩色圆底（颜色表示严重程度）+ 白色图标（形状表示具体原因），鼠标点它，小鲸鱼会用呆萌口吻说当前出了什么事（例如「余额只剩 ¥3.20 啦...」），再点一下小声给个建议（例如「充点钱，或调低 ¥5.00 告警线~」），来回切换。

| 角标 | 代表 | 颜色 |
| --- | --- | --- |
| Wi-Fi + 斜杠 | 取数失败：网络不通 | 红 |
| 钥匙 + 斜杠 | 取数失败：没配密钥 | 红 |
| 感叹号 | 取数失败：其它错误 | 红 |
| ¥ + 下箭头 | 余额低于告警线 | 红 |
| ¥ + 上箭头 | 今日用量超过预算 | 橙 |
...
多个原因同时存在时按 红 > 橙 > 蓝 显示一个。气泡只有正文 + 一行小字提示（`再点一下，有建议~` / `再点一下，看原因~`），没有标题行；正文和提示都是加粗的，正文 56 号深蓝、提示 40 号浅蓝。
...
**托盘与通知**：挂件常驻一个托盘图标（鲸鱼头像）。**左键单击托盘 = 打开并前置悬浮窗**；右键托盘是 **显示/关闭悬浮窗（按当前状态切换文案）｜ 开机自启 ｜ 自动检查更新 ｜（有新版本，检测到才出现）｜ 退出**。悬浮窗菜单里的「关闭悬浮窗」只是把窗口收起来（托盘仍在），下次启动挂件会照常显示悬浮窗——想彻底退出请用托盘菜单的「退出」。需要告知你的事情（取数失败、余额告警、今日预算超支、更新提示）会走同一条通道：鲸鱼右上角角标（红=取数失败或余额告警、橙=今日预算超支、蓝=有新版本）+ 一次 Windows 托盘气泡，事件记在 `events.json`（最多 10 条）；同一个来源 10 分钟内只弹一次气泡。托盘气泡可以在菜单里关掉，角标与事件记录不受影响。右键菜单「最近事件」列出最近 5 条（点条目打开事件目录）；取数失败期间角标常亮，恢复成功后自动清除并记一条恢复事件。**峰谷切换只弹气泡与托盘通知，不进角标、不计入事件；音效播放器的自动重建只写日志，同样不进角标与事件。**
