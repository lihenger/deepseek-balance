...
**托盘与通知**：挂件常驻一个托盘图标（鲸鱼头像）。双击托盘切换悬浮窗显示，右键托盘可 立即刷新 / 开机自启 / 只留托盘 / 退出。需要告知你的事情（取数失败、自愈动作、余额告警、峰谷切换、更新提示）会走同一条通道：鲸鱼右上角角标（红=取数失败或余额告警、橙=自愈事件、蓝=有新版本/峰谷切换）+ 一次 Windows 托盘气泡，事件记在 `events.json`（最多 50 条）；同一个来源 10 分钟内只弹一次气泡。托盘气泡可以在菜单里关掉，角标与事件记录不受影响。右键菜单「最近事件」列出最近 5 条（点条目打开事件目录）；取数失败期间角标常亮，恢复成功后自动清除并记一条恢复事件。
