...
**右键菜单**（自上而下）：立即刷新 → 大小滑块 → 音效开关 / 音量 / 音效集 → 用量 → 峰谷文案 → 告警（余额告警开关 + 阈值、今日预算开关 + 阈值、峰谷切换提醒）→ 最近事件 → 气泡 → **关闭悬浮窗**（最下方）。开机自启与自动检查更新不在悬浮窗菜单里，都在托盘菜单。设置写在 `%USERPROFILE%\.codex\deepseek-balance\widget.json`。
...
**托盘与通知**：挂件常驻一个托盘图标（鲸鱼头像）。**左键单击托盘 = 打开并前置悬浮窗**；右键托盘是 **显示/关闭悬浮窗（按当前状态切换文案）｜ 开机自启 ｜ 自动检查更新 ｜（有新版本，检测到才出现）｜ 退出**。悬浮窗菜单里的「关闭悬浮窗」只是把窗口收起来（托盘仍在），下次启动挂件会照常显示悬浮窗——想彻底退出请用托盘菜单的「退出」。需要告知你的事情（取数失败、自愈动作、余额告警、峰谷切换、更新提示）会走同一条通道：鲸鱼右上角角标（红=取数失败或余额告警、橙=自愈事件、蓝=有新版本/峰谷切换）+ 一次 Windows 托盘气泡，事件记在 `events.json`（最多 10 条）；同一个来源 10 分钟内只弹一次气泡。托盘气泡可以在菜单里关掉，角标与事件记录不受影响。右键菜单「最近事件」列出最近 5 条（点条目打开事件目录）；取数失败期间角标常亮，恢复成功后自动清除并记一条恢复事件。
