...
**右键菜单**：大小滑块、音效开关与音量、音效集（小黄鸭 / 音效1 / 重载音效）、用量模式（小鲸鱼记账 / 实时·令牌）、峰谷文案（默认 / 梁文峰谷 / !?强强?!）、告警（余额告警 / 今日预算 / 峰谷切换提醒 / 自定义阈值）、最近事件、自动检查更新、气泡开关、关闭悬浮窗、开机自启、立即刷新。退出只在托盘菜单里。设置写在 `%USERPROFILE%\.codex\deepseek-balance\widget.json`。

**续航与提醒**：气泡第三行显示 `今日已用 ¥x.xx · 可用约 N 天`（按最近小时分桶的消耗速率估算，样本不足时退回今日均值，估不出来就不显示）。峰谷切换前 15 分钟提醒一次、切换瞬间弹一次气泡与通知；余额低于告警线（默认 ¥5，6 小时内只提醒一次）或今日用量超过预算（默认关）时同样会提醒——除了 3/5/10/20 这类档位，右键菜单「告警 → 自定义…」还能直接填任意金额（0 表示关闭，最多两位小数）。告警关闭、阈值调高或条件不再成立时，红/橙角标会立即消失。

**更新检查**：启动 2 分钟后检查一次、之后每 6 小时一次，比对本地插件仓库 `%USERPROFILE%\plugins\deepseek-balance` 的 HEAD 与 GitHub 仓库 `main` 的 commit。凭据优先取环境变量 `GITHUB_TOKEN`，没有就用本机 git 已保存的 GitHub 凭据；两者都没有就只写日志、不打扰。发现不一致时发一次蓝色通知，并在右键菜单显示「有新版本 本地 xxx → 远端 xxx」，点一下打开仓库页——只提示，不自动下载或覆盖文件。用不到的话在右键菜单取消「自动检查更新」。

**托盘与通知**：挂件常驻一个托盘图标（鲸鱼头像）。**左键单击托盘 = 打开并前置悬浮窗**；右键托盘可 立即刷新 / 开机自启 / 显示悬浮窗 / 退出。悬浮窗菜单里的「关闭悬浮窗」只是把窗口收起来（托盘仍在），下次启动挂件会照常显示悬浮窗——想彻底退出请用托盘菜单的「退出」。需要告知你的事情（取数失败、自愈动作、余额告警、峰谷切换、更新提示）会走同一条通道：鲸鱼右上角角标（红=取数失败或余额告警、橙=自愈事件、蓝=有新版本/峰谷切换）+ 一次 Windows 托盘气泡，事件记在 `events.json`（最多 10 条）；同一个来源 10 分钟内只弹一次气泡。托盘气泡可以在菜单里关掉，角标与事件记录不受影响。右键菜单「最近事件」列出最近 5 条（点条目打开事件目录）；取数失败期间角标常亮，恢复成功后自动清除并记一条恢复事件。
