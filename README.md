...
**角标怎么用**：鼠标点鲸鱼右上角的角标，气泡会用小鲸鱼的口吻说当前出了什么事（例如「余额只剩 ¥3.20 啦...」），再点一下它小声给个建议（例如「充点钱，或调低 ¥5.00 告警线~」），来回切换。气泡只有正文 + 一行小字提示（`再点一下，有建议~` / `再点一下，看原因~`），没有标题行；正文和提示都是加粗的，正文 56 号深蓝、提示 40 号浅蓝。
